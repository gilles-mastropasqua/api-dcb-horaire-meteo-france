...
import { convertToISODate, FieldType, sanitizeNumPoste, sanitizeValue } from '@/app/api/update/utils/utils';

/**
 * Defines the structure of observation fields.
 */
...
const SOURCE_FIELDS: ObservationField[] = [
...
/**
 * Processed observation fields, ensuring proper naming conventions.
 */
export const OBSERVATION_FIELDS = [
    ...SOURCE_FIELDS.map((field) => ({
        source: field.source,
        name: field.name ?? camelCase(field.source),
        type: field.type,
    })),
    {
        source: 'AAAAMMJJHH',
...
/**
 * Generates a PostgreSQL-compatible column list.
 */
export const PG_COLUMNS: string = OBSERVATION_FIELDS.map(({ name }) => `"${name}"`).join(', ');

/**
 * Normalizes an observation record from a CSV file.
 * @param record The raw CSV record.
 * @returns A normalized object with sanitized values.
 */
export function normalizeObservationRecord(record: Record<string, unknown>): Record<string, string | number | null> {
